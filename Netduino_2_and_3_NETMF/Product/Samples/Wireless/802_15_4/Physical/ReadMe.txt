...
1. Open two instances of IEEE_802_15_4_PHY.sln in Visual Studio 2008.
...
3. In file Program.cs, set a breakpoint on the TestSend method and the
   phy_DataIndicationAqc method.
...
6. Go back to the first instance and change at runtime the boolean parameter 
   passed to TestSend, so that the device you are debugging will act as a receiver

7. Step through the code and debug the first device while it receives frames from 
   the first device.
