...
-  Using the most basic features of touch screen.
-  Handling the touch screen events.


This sample runs on the following devices:
-  The .NET Micro Framework SDK 4.0 Emulator or later.
-  Any physical device that implements the Touch and Gesture API and provides a valid
...
1. Open TouchScreen.sln in Visual Studio 2008.

2. In Solution Explorer, right-click the Solution node (not the project node) 
   and then click Properties.  The Solution 'TouchScreen' Property Page 
   dialog box appears.
...
8. Open the file Program.cs.  Set a breakpoint on the methods TouchUpHandler and 
   GestureChnagedHandler.  

9. Press F5 to start debugging.  When the text "Click Anywhere" appears, click, 
   drag, and release the mouse button to cause touch events and gestures to occur.
   Click on the bitmaps or try to cause left and right gestures by clicking and 
   dragging the mouse.  The breakpoints are hit.  
