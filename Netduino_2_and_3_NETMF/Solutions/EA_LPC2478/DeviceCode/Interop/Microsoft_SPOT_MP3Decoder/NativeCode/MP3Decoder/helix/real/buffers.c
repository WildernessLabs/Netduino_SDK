...
extern unsigned char static_mp3DecInfo[sizeof(MP3DecInfo)];
extern unsigned char static_fh[sizeof(FrameHeader)];
extern unsigned char static_si[sizeof(SideInfo)];
extern unsigned char static_sfi[sizeof(ScaleFactorInfo)];
extern unsigned char static_hi[sizeof(HuffmanInfo)];
extern unsigned char static_di[sizeof(DequantInfo)];
extern unsigned char static_mi[sizeof(IMDCTInfo)];
extern unsigned char static_sbi[sizeof(SubbandInfo)];
...
MP3DecInfo *AllocateBuffers(void)
{
	...
	MP3DecInfo *mp3DecInfo = 	(MP3DecInfo *) static_mp3DecInfo;
	/*FrameHeader *fh =  			(FrameHeader *) static_fh;
	SideInfo *si =  			(SideInfo *) static_si;
	ScaleFactorInfo *sfi = 		(ScaleFactorInfo *) static_sfi;
	HuffmanInfo *hi = 			(HuffmanInfo *) static_hi;
	DequantInfo *di = 			(DequantInfo *) static_di;
	IMDCTInfo *mi =				(IMDCTInfo *) static_mi;
	SubbandInfo *sbi =			(SubbandInfo *) static_sbi;*/
	
		
	/*mp3DecInfo = (MP3DecInfo *)malloc(sizeof(MP3DecInfo));
	if (!mp3DecInfo)
		return 0;*/
	ClearBuffer(static_mp3DecInfo, sizeof(MP3DecInfo));
	...
	*/
	mp3DecInfo->FrameHeaderPS =     (void *)static_fh;
	mp3DecInfo->SideInfoPS =        (void *)static_si;
	mp3DecInfo->ScaleFactorInfoPS = (void *)static_sfi;
	mp3DecInfo->HuffmanInfoPS =     (void *)static_hi;
	mp3DecInfo->DequantInfoPS =     (void *)static_di;
	mp3DecInfo->IMDCTInfoPS =       (void *)static_mi;
	mp3DecInfo->SubbandInfoPS =     (void *)static_sbi;
	...
	*/
	...
	ClearBuffer(static_fh,  sizeof(FrameHeader));
	ClearBuffer(static_si,  sizeof(SideInfo));
	ClearBuffer(static_sfi, sizeof(ScaleFactorInfo));
	ClearBuffer(static_hi,  sizeof(HuffmanInfo));
	ClearBuffer(static_di,  sizeof(DequantInfo));
	ClearBuffer(static_mi,  sizeof(IMDCTInfo));
	ClearBuffer(static_sbi, sizeof(SubbandInfo));
