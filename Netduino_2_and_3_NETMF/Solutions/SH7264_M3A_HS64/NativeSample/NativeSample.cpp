...
BLOCK_CONFIG        * pBLOCK_CONFIG_NOR         = &g_BS_Config_SH7264_M3A_HS64;
...
void VDC3_Rect_Write (unsigned short *lcd_top_addr, LCD_RECT data)
{
    ...
    unsigned short  Width = g_SH7264VDC3_Config.Width;
    unsigned short  Height = g_SH7264VDC3_Config.Height;

    if ((data.Left_top.p_x < 0) || (Width < data.Left_top.p_x) ||
        (data.Right_bottom.p_x <= 0 ) || (Width < data.Right_bottom.p_x) ||
        (data.Left_top.p_x == data.Right_bottom.p_x))
    {
        return;
    }
    if ((data.Left_top.p_y < 0) || (Height < data.Left_top.p_y) ||
        (data.Right_bottom.p_y <= 0 ) || (Height < data.Right_bottom.p_y) ||
        (data.Left_top.p_y == data.Right_bottom.p_y))
    {
        return;
    }

    addr = (unsigned short *)((unsigned long)lcd_top_addr + (data.Left_top.p_y * 2* Width) + (data.Left_top.p_x * 2));
    save_addr = addr;
    for (p_y = data.Left_top.p_y; p_y < data.Right_bottom.p_y; p_y++)
    {
        ...
        save_addr = (unsigned short *)((unsigned long)save_addr + (2*Width));
        addr = save_addr;
    }
...
void ApplicationEntryPoint()
{
    ...
    do
    {
        lcd_printf( "\n\nSH7264 M3A-HS64 .NET Micro Framework\r\n" );
        ...
        do
        {
            Events = Events_WaitForEvents( SYSTEM_EVENT_FLAG_COM_IN, 50 );
