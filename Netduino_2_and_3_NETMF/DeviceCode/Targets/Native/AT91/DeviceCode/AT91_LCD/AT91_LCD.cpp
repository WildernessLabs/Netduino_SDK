...
BOOL AT91_LCD_Driver::Initialize(DISPLAY_CONTROLLER_CONFIG& config)
{
    ...
    pmc.EnableSystemClock(AT91C_PMC_HCK1);    
    ...
    lcdc.LCDC_FRMCFG = (15 << 24) + (config.Height * config.Width * config.BitsPerPixel >> 5);
...
{    
    // Turn off LCDC
    AT91_PMC &pmc = AT91::PMC();
    pmc.DisableSystemClock(AT91C_PMC_HCK1);    
...
BOOL AT91_LCD_Driver::Enable(BOOL fEnable)
{
    ...
    if(fEnable)
    {
        ...
        lcdc.LCDC_PWRCON    = AT91_LCDC::LCDC_PWR | 0x0c;
    }
    else
    {
        lcdc.LCDC_PWRCON = 0x0C;
