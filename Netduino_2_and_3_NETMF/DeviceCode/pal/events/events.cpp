...
#pragma arm section zidata = "g_Events_BoolTimerCompletion"
#endif
...
void Events_Set( UINT32 Events )
{
    ...
    if(g_Event_Callback) g_Event_Callback( g_Event_Callback_Arg );
}
...
UINT32 Events_Get( UINT32 EventsOfInterest )
{
    ...
    // give the caller notice of just the events they asked for
    Events        = SystemEvents & EventsOfInterest;
    // remove the events being sent to the caller
...
{
    NATIVE_PROFILE_PAL_EVENTS();
    // if the event the user was waiting for caused us to wakeup,
