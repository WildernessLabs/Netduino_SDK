...
namespace Microsoft.SPOT.Debugger
{
    public class CRC
    {
        static uint[] c_CRCTable = new uint[256]
        {
        ...
        static public uint ComputeCRC( byte[] buf, int offset, int length, uint crc )
        {
            while(length-- > 0)
            {
                crc = c_CRCTable[((crc >> 24) ^ (buf[offset++])) & 0xFF] ^ (crc << 8);
            }

            return crc;
        }

        static public uint ComputeCRC( byte val, uint crc )
        {
            return c_CRCTable[((crc >> 24) ^ (val)) & 0xff] ^ (crc << 8);
        }

        static public uint ComputeCRC( byte[] buf, uint crc )
        {
            return ComputeCRC( buf, 0, buf.Length, crc );
        }
    }
