...
bool CLR_DBG_Debugger::AccessMemory( CLR_UINT32 location, UINT32 lengthInBytes, BYTE* buf, int mode )
{
    ...
    //--// RAM write
        ...
        {
#if !defined(BUILD_RTM) && !(defined(PLATFORM_ARM_Netduino3Wifi) || defined(PLATFORM_ARM_Netduino3Ethernet) || defined(PLATFORM_ARM_Netduino3) || defined(PLATFORM_ARM_NetduinoPlus2) || defined(PLATFORM_ARM_NetduinoGo))
            CLR_Debug::Printf(" Debugger attempting to access invalid address %x and range %x Ram Start %x, Ram end %x\r\n", sectAddr, lengthInBytes, ramStartAddress, ramEndAddress);
#endif
            return FALSE;
        }
        else
