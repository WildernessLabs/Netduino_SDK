...
{
    return PXA271_SPI_Driver::Initialize();
}

void CPU_SPI_Uninitialize()
{
    PXA271_SPI_Driver::Uninitialize();
}
...
{
    return PXA271_SPI_Driver::nWrite16_nRead16( Configuration, Write16, WriteCount, Read16, ReadCount, ReadStartOffset );
}
...
{
    return PXA271_SPI_Driver::nWrite8_nRead8( Configuration, Write8, WriteCount, Read8, ReadCount, ReadStartOffset );
}

BOOL CPU_SPI_Xaction_Start( const SPI_CONFIGURATION& Configuration )
{
    return PXA271_SPI_Driver::Xaction_Start( Configuration );
}

BOOL CPU_SPI_Xaction_Stop( const SPI_CONFIGURATION& Configuration )
{
    return PXA271_SPI_Driver::Xaction_Stop( Configuration );
}

BOOL CPU_SPI_Xaction_nWrite16_nRead16( SPI_XACTION_16& Transaction )
{
    return PXA271_SPI_Driver::Xaction_nWrite16_nRead16( Transaction );
}

BOOL CPU_SPI_Xaction_nWrite8_nRead8( SPI_XACTION_8& Transaction )
{
    return PXA271_SPI_Driver::Xaction_nWrite8_nRead8( Transaction );
}

UINT32 CPU_SPI_PortsCount()
{
    return PXA271_SPI::c_MAX_SPI;
}

void   CPU_SPI_GetPins( UINT32 spi_mod, GPIO_PIN& msk, GPIO_PIN& miso, GPIO_PIN& mosi )
{
    return PXA271_SPI_Driver::GetPins( spi_mod, msk, miso, mosi );
}
