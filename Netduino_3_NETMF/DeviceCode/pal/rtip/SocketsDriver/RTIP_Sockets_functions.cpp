...
BOOL HAL_SOCK_Initialize()
{
    ...
    return RTIP_SOCKETS_Driver::Initialize();
}

BOOL HAL_SOCK_Uninitialize()
{
    NATIVE_PROFILE_PAL_NETWORK();
    return RTIP_SOCKETS_Driver::Uninitialize();
}
...
{
    NATIVE_PROFILE_PAL_NETWORK();
    return RTIP_SOCKETS_Driver::UpdateAdapterConfiguration(interfaceIndex, updateFlags, config);
}

HRESULT HAL_SOCK_CONFIGURATION_LoadWirelessConfiguration( UINT32 interfaceIndex, SOCK_WirelessConfiguration* wirelessConfig )
{
    NATIVE_PROFILE_PAL_NETWORK();
    return RTIP_SOCKETS_Driver::LoadWirelessConfiguration(interfaceIndex, wirelessConfig);
}
