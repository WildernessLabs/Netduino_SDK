...
static void UnpackSFMPEG1(BitStreamInfo *bsi, SideInfoSub *sis, ScaleFactorInfoSub *sfis, int *scfsi, int gr, ScaleFactorInfoSub *sfisGr0)
{
	...
	if (sis->blockType == 2) {
		...
		if (sis->mixedBlock) {          
			...
			for (sframeb = 0; sframeb < 8; sframeb++)
				sfis->l[sframeb] =    (char)GetBits(bsi, slen0);
			sframeb = 3;
		} else {
			/* all short blocks */
			sframeb = 0;
		...
		for (      ; sframeb < 6; sframeb++) {
			sfis->s[sframeb][0] = (char)GetBits(bsi, slen0);
			sfis->s[sframeb][1] = (char)GetBits(bsi, slen0);
			sfis->s[sframeb][2] = (char)GetBits(bsi, slen0);
		}

		for (      ; sframeb < 12; sframeb++) {
			sfis->s[sframeb][0] = (char)GetBits(bsi, slen1);
			sfis->s[sframeb][1] = (char)GetBits(bsi, slen1);
			sfis->s[sframeb][2] = (char)GetBits(bsi, slen1);
		}
		...
		if(gr == 0) {
			/* first granule */
			for (sframeb = 0;  sframeb < 11; sframeb++) 
				sfis->l[sframeb] = (char)GetBits(bsi, slen0);
			for (sframeb = 11; sframeb < 21; sframeb++) 
				sfis->l[sframeb] = (char)GetBits(bsi, slen1);
			return;
		} else {
			/* second granule
			 ...
			 */
			sframeb = 0;
			if(scfsi[0])  for(  ; sframeb < 6 ; sframeb++) sfis->l[sframeb] = sfisGr0->l[sframeb];
			else          for(  ; sframeb < 6 ; sframeb++) sfis->l[sframeb] = (char)GetBits(bsi, slen0);
			if(scfsi[1])  for(  ; sframeb <11 ; sframeb++) sfis->l[sframeb] = sfisGr0->l[sframeb];
			else          for(  ; sframeb <11 ; sframeb++) sfis->l[sframeb] = (char)GetBits(bsi, slen0);
			if(scfsi[2])  for(  ; sframeb <16 ; sframeb++) sfis->l[sframeb] = sfisGr0->l[sframeb];
			else          for(  ; sframeb <16 ; sframeb++) sfis->l[sframeb] = (char)GetBits(bsi, slen1);
			if(scfsi[3])  for(  ; sframeb <21 ; sframeb++) sfis->l[sframeb] = sfisGr0->l[sframeb];
			else          for(  ; sframeb <21 ; sframeb++) sfis->l[sframeb] = (char)GetBits(bsi, slen1);
		}
		/* last sf band not transmitted */
		sfis->l[21] = 0;
		sfis->l[22] = 0;
 ...
 *   partition: scale factor groups (sframeb1 through sframeb4)
...
static void UnpackSFMPEG2(BitStreamInfo *bsi, SideInfoSub *sis, ScaleFactorInfoSub *sfis, int gr, int ch, int modeExt, ScaleFactorJS *sfjs)
{
	...
	int i, sframeb, sfcIdx, btIdx, nrIdx, iipTest;
	int slen[4], nr[4];
	...
	if(sis->blockType == 2) {
		if(sis->mixedBlock) {
			/* do long block portion */
			iipTest = (1 << slen[0]) - 1;
			for (sframeb=0; sframeb < 6; sframeb++) {
				sfis->l[sframeb] = (char)GetBits(bsi, slen[0]);
			}
			sframeb = 3;  /* start sframeb for short */
			nrIdx = 1;
		} else {      
			/* all short blocks, so start nr, sframeb at 0 */
			sframeb = 0;
			nrIdx = 0;
		}

		/* remaining short blocks, sframeb just keeps incrementing */
		for (    ; nrIdx <= 3; nrIdx++) {
			iipTest = (1 << slen[nrIdx]) - 1;
			for (i=0; i < nr[nrIdx]; i++, sframeb++) {
				sfis->s[sframeb][0] = (char)GetBits(bsi, slen[nrIdx]);
				sfis->s[sframeb][1] = (char)GetBits(bsi, slen[nrIdx]);
				sfis->s[sframeb][2] = (char)GetBits(bsi, slen[nrIdx]);
			}
		}
		/* last sf band not transmitted */
		sfis->s[12][0] = sfis->s[12][1] = sfis->s[12][2] = 0;
		...
		for (nrIdx = 0; nrIdx <= 3; nrIdx++) {
			iipTest = (1 << slen[nrIdx]) - 1;
			for(i=0; i < nr[nrIdx]; i++, sframeb++) {
				sfis->l[sframeb] = (char)GetBits(bsi, slen[nrIdx]);
			}
		}
		/* last sf band not transmitted */
