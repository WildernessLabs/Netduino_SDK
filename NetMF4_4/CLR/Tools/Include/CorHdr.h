...
#define mdGenericPar mdGenericParam
...
typedef struct IMAGE_COR20_HEADER
{
    ...
    IMAGE_DATA_DIRECTORY    StrongNameSignature;            // blob of bytes

    // Regular fixup and binding information
    IMAGE_DATA_DIRECTORY    CodeManagerTable;           // I believe that this is unused (was for managed native code)
    IMAGE_DATA_DIRECTORY    VTableFixups;
    IMAGE_DATA_DIRECTORY    ExportAddressTableJumps;

    // Precompiled image info (internal use only - set to zero)
    IMAGE_DATA_DIRECTORY    ManagedNativeHeader;        // points at a CORCOMPILE_HEADER

} IMAGE_COR20_HEADER;

#else // !__IMAGE_COR20_HEADER_DEFINED__

// This define is required because we pull in the COM+ 2.0 PE header
// definition from winnt.h, and the constant below hasn't propogated yet. 
...
typedef enum CorAssemblyFlags
{
    ...
    afLongevityUnspecified  =   0x0000,     // Nothing set.
    afLibrary               =   0x0002,     // All types in the assembly are Library types.
    afAppDomainPlatform     =   0x0004,     // All types in the assembly are Platform types.
    afProcessPlatform       =   0x0006,     // All types in the assembly are Platform types.
    afSystemPlatform        =   0x0008,     // All types in the assembly are Platform types.
    afLongevityMask         =   0x000E,     // Bits describing the platform/library property of the assembly.
...
// Macros for accessing the longevity flags of CorAssemblyFlags.
#define IsAfLongevityUnspecified(x)         (((x) & afLongevityMask) == afLongevityUnspecified)
#define IsAfLibrary(x)                      (((x) & afLongevityMask) == afLibrary)
#define IsAfAppDomainPlatform(x)            (((x) & afLongevityMask) == afAppDomainPlatform)
#define IsAfProcessPlatform(x)              (((x) & afLongevityMask) == afProcessPlatform)
#define IsAfSystemPlatform(x)               (((x) & afLongevityMask) == afSystemPlatform)
...
#define ELEMENT_TYPE_WITH ELEMENT_TYPE_GENERICINST
...
#define mdtGenericPar mdtGenericParam
...
#define mdGenericParNil mdGenericParamNil
...
#define IsOfReadOnly(x)                     (((x) & ofReadWriteMask) == ofRead)
