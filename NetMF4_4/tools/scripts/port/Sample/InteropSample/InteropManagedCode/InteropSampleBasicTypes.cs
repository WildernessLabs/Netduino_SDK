...
namespace Microsoft.SPOT.Interop
{
 ...
 public static class TestBasicTypes
  ...
  public class TestCallback : NativeEventDispatcher       
  {
      ...
      {
          m_CallbackReceived++;
          Debug.Print( "Callback Received: " + m_CallbackReceived );
      }
      ...
      public bool TestCallbacks()
      {
          ...
          {
              GenerateInterrupt();
              Debug.Print( "Callback posted: " + ( clbPosted + 1 ) );
          }

          // Allows all callbacks to complete.
          System.Threading.Thread.Sleep( 1000 );

          Debug.Print( "Total Posted: " + m_CallbackRequired + ", Total Received: " + m_CallbackReceived );

          if ( m_CallbackRequired == m_CallbackReceived )
          {
              Debug.Print( "Testing of callbacks succeded\n" );
          }
          else
          {
              Debug.Print( "Error in testing of callbacks\n" );
          }
