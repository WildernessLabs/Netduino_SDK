...
static void transform_3 (byte *data) 
	...
	TINYCLR_SSL_MEMMOVE(data,data+2,30);
	data[30]=acc&0xff;
	data[31]=acc>>8;
	}
...
int init_gost_hash_ctx(gost_hash_ctx *ctx, const gost_subst_block *subst_block)
	{	
	TINYCLR_SSL_MEMSET(ctx,0,sizeof(gost_hash_ctx));
...
int start_hash(gost_hash_ctx *ctx)
	...
	TINYCLR_SSL_MEMSET(&(ctx->H),0,32);
	TINYCLR_SSL_MEMSET(&(ctx->S),0,32);
	ctx->len = 0L;
	ctx->left=0;
	return 1;
	}
