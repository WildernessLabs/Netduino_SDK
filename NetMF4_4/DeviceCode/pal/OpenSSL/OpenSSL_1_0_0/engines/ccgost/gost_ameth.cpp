...
static ASN1_STRING  *encode_gost_algor_params(const EVP_PKEY *key)
	...
	switch (EVP_PKEY_base_id(key)) 
		...
		case NID_id_GostR3410_2001:
			pkey_param_nid = EC_GROUP_get_curve_name(EC_KEY_get0_group((const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)key)));
			break;
		case NID_id_GostR3410_94:
			pkey_param_nid = (int) gost94_nid_by_params((DSA*)EVP_PKEY_get0((EVP_PKEY *)key));
...
static int print_gost_94(BIO *out, const EVP_PKEY *pkey, int indent,
	...
	param_nid = gost94_nid_by_params((DSA*)EVP_PKEY_get0((EVP_PKEY *)pkey));
	BIO_indent(out,indent,128);
...
static int print_gost_01(BIO *out, const EVP_PKEY *pkey, int indent,
	...
	param_nid = EC_GROUP_get_curve_name(EC_KEY_get0_group((EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)pkey)));
	if (!BIO_indent(out,indent,128)) return 0;
	BIO_printf(out,"Parameter set: %s\n",OBJ_nid2ln(param_nid));
	return 1;
}
...
static int param_missing_gost94(const EVP_PKEY *pk) 
	{
	const DSA *dsa = (const DSA*)EVP_PKEY_get0((EVP_PKEY *)pk);
	if (!dsa) return 1;
	if (!dsa->q) return 1;
	return 0;
	}
...
static int param_missing_gost01(const EVP_PKEY *pk) 
	{
	const EC_KEY *ec = (const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)pk);
	if (!ec) return 1;
...
static int param_copy_gost94(EVP_PKEY *to, const EVP_PKEY *from) 
	{
	const DSA *dfrom = (const DSA*)EVP_PKEY_get0((EVP_PKEY *)from);
	DSA *dto = (DSA*)EVP_PKEY_get0(to);
...
static int param_copy_gost01(EVP_PKEY *to, const EVP_PKEY *from) 
	{
	EC_KEY *eto = (EC_KEY*)EVP_PKEY_get0(to);
	const EC_KEY *efrom = (const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)from);
	if (EVP_PKEY_base_id(from) != EVP_PKEY_base_id(to)) 
		{
...
static int param_cmp_gost94(const EVP_PKEY *a, const EVP_PKEY *b) 
	{
	const DSA *da = (const DSA*)EVP_PKEY_get0((EVP_PKEY *)a);
	const DSA *db = (const DSA*)EVP_PKEY_get0((EVP_PKEY *)b);
...
static int param_cmp_gost01(const EVP_PKEY *a, const EVP_PKEY *b) 
	{
	if (EC_GROUP_get_curve_name(EC_KEY_get0_group((const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)a)))==
		EC_GROUP_get_curve_name(EC_KEY_get0_group((const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)b)))) 
		{
		return 1;
		}
	return 0;
...
static int pub_decode_gost94(EVP_PKEY *pk, X509_PUBKEY *pub)
	...
	databuf = (unsigned char*)OPENSSL_malloc(octet->length);
	for (i=0,j=octet->length-1;i<octet->length;i++,j--)
		{
		databuf[j]=octet->data[i];
		}	
	dsa = (DSA*)EVP_PKEY_get0(pk);
...
static int pub_encode_gost94(X509_PUBKEY *pub,const EVP_PKEY *pk)
	...
	int ptype = V_ASN1_UNDEF;
	DSA *dsa = (DSA*)EVP_PKEY_get0((EVP_PKEY *)pk);
	...
	databuf = (unsigned char*)OPENSSL_malloc(data_len);
...
static int pub_decode_gost01(EVP_PKEY *pk,X509_PUBKEY *pub)
	...
	EVP_PKEY_assign(pk,OBJ_obj2nid(palgobj),NULL);	
	if (!decode_gost_algor_params(pk,palg)) return 0;
	group = EC_KEY_get0_group((const EC_KEY*)EVP_PKEY_get0(pk));
	octet = d2i_ASN1_OCTET_STRING(NULL,&pubkey_buf,pub_len);
	if (!octet) 
		{
	...
	BN_free(X);
	BN_free(Y);
	if (!EC_KEY_set_public_key((EC_KEY*)EVP_PKEY_get0(pk),pub_key))
		{
		GOSTerr(GOST_F_PUB_DECODE_GOST01,
			ERR_R_EC_LIB);
		EC_POINT_free(pub_key);
...
static int pub_encode_gost01(X509_PUBKEY *pub,const EVP_PKEY *pk)
	...
	BN_free(order);
	databuf = (unsigned char*)OPENSSL_malloc(data_len);
	TINYCLR_SSL_MEMSET(databuf,0,data_len);
...
static int pub_cmp_gost01(const EVP_PKEY *a,const EVP_PKEY *b)
	{
	const EC_KEY *ea = (const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)a);
	const EC_KEY *eb = (const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)b);
...
{
   int nid=gost94_nid_by_params((DSA*)EVP_PKEY_get0((EVP_PKEY *)pkey));
   return i2d_ASN1_OBJECT(OBJ_nid2obj(nid),pder);
}
static int gost2001_param_encode(const EVP_PKEY *pkey, unsigned char **pder) 
{
   int nid = EC_GROUP_get_curve_name(EC_KEY_get0_group((const EC_KEY*)EVP_PKEY_get0((EVP_PKEY *)pkey)));
   return i2d_ASN1_OBJECT(OBJ_nid2obj(nid),pder);
}
