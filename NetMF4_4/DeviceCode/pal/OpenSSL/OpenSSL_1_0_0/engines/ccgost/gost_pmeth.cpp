...
static int pkey_gost_copy(EVP_PKEY_CTX *dst, EVP_PKEY_CTX *src)
	...
	src_data = (gost_pmeth_data*)EVP_PKEY_CTX_get_data(src);
	dst_data = (gost_pmeth_data*)EVP_PKEY_CTX_get_data(dst);
	*dst_data = *src_data;
...
static int pkey_gost_ctrl(EVP_PKEY_CTX *ctx, int type, int p1, void *p2)
	...
	switch (type)
		...
		case EVP_PKEY_CTRL_SET_IV:
			pctx->shared_ukm=(unsigned char*)OPENSSL_malloc((int)p1);
			TINYCLR_SSL_MEMCPY(pctx->shared_ukm,p2,(int) p1);
			return 1;
...
static int pkey_gost_ctrl94_str(EVP_PKEY_CTX *ctx,
	...
	{
	int param_nid=0;
	if(!TINYCLR_SSL_STRCMP(type, param_ctrl_string))
		{
		if (!value)
			{
			return 0;
			}
		if (TINYCLR_SSL_STRLEN(value) == 1)
			{
			switch(TINYCLR_SSL_TOUPPER(value[0]))
				{
				case 'A':
		...
		else if ((TINYCLR_SSL_STRLEN(value) == 2) && (TINYCLR_SSL_TOUPPER(value[0]) == 'X'))
			{
			switch (TINYCLR_SSL_TOUPPER(value[1]))
				{
				case 'A':
...
static int pkey_gost94_cp_sign(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
	...
	unpacked_sig = gost_do_sign(tbs,tbs_len,(DSA*)EVP_PKEY_get0(pkey));
	if (!unpacked_sig)
		{
		return 0;
		}
...
static int pkey_gost01_cp_sign(EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
	...
	unpacked_sig = gost2001_do_sign(tbs,tbs_len,(EC_KEY*)EVP_PKEY_get0(pkey));
	if (!unpacked_sig)
		{
		return 0;
		}
...
static int pkey_gost94_cp_verify(EVP_PKEY_CTX *ctx, const unsigned char *sig,
	...
	if (!s) return 0;
	if (pub_key) ok = gost_do_verify(tbs,tbs_len,s,(DSA*)EVP_PKEY_get0(pub_key));
	DSA_SIG_free(s);
	return ok;
	}
...
static int pkey_gost_mac_init(EVP_PKEY_CTX *ctx)
	...
	data =(gost_mac_pmeth_data*) OPENSSL_malloc(sizeof(struct gost_mac_pmeth_data));
	if (!data) return 0;
	TINYCLR_SSL_MEMSET(data,0,sizeof(struct gost_mac_pmeth_data));
	EVP_PKEY_CTX_set_data(ctx,data);
	return 1;
	}	
static void pkey_gost_mac_cleanup (EVP_PKEY_CTX *ctx)
	{
	struct gost_mac_pmeth_data *data = (gost_mac_pmeth_data*)EVP_PKEY_CTX_get_data(ctx);
	OPENSSL_free(data);
	}	
static int pkey_gost_mac_copy(EVP_PKEY_CTX *dst, EVP_PKEY_CTX *src)
	...
	src_data = (gost_mac_pmeth_data*)EVP_PKEY_CTX_get_data(src);
	dst_data = (gost_mac_pmeth_data*)EVP_PKEY_CTX_get_data(dst);
	*dst_data = *src_data;
	return 1;
	}
...
static int pkey_gost_mac_ctrl_str(EVP_PKEY_CTX *ctx,
	const char *type, const char *value)
	{
	if (!TINYCLR_SSL_STRCMP(type, key_ctrl_string)) 
		{
		if (TINYCLR_SSL_STRLEN(value)!=32) 
			{
	...
	if (!TINYCLR_SSL_STRCMP(type, hexkey_ctrl_string)) 
		{
			long keylen; int ret;
...
static int pkey_gost_mac_keygen(EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
	{
		...
		keydata = (unsigned char*)OPENSSL_malloc(32);
		TINYCLR_SSL_MEMCPY(keydata,data->key,32);
