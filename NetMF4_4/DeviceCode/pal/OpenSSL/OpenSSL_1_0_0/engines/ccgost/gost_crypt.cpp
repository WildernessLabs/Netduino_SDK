...
const struct gost_cipher_info *get_encryption_params(ASN1_OBJECT *obj)
	...
	if (!obj)
		...
		if (!params || !TINYCLR_SSL_STRLEN(params)) 
...
static int gost_cipher_init_param(EVP_CIPHER_CTX *ctx, const unsigned char *key,
	...
	{
	struct ossl_gost_cipher_ctx *c=(ossl_gost_cipher_ctx*)ctx->cipher_data;
...
static void gost_crypt_mesh (void *ctx,unsigned char *iv,unsigned char *buf)
	{
	struct ossl_gost_cipher_ctx *c = (ossl_gost_cipher_ctx *)ctx;
...
/* process partial block if any */
	if (ctx->num) 
		...
		if (j==8)
			{
			TINYCLR_SSL_MEMCPY(ctx->iv,ctx->buf+8,8);
			ctx->num=0;
			}
		else
			{
	...
	for (;i+8<inl;i+=8,in_ptr+=8,out_ptr+=8)
		...
		/*output this block */
		if (!ctx->encrypt) TINYCLR_SSL_MEMCPY(ctx->iv,in_ptr,8);
		for (j=0;j<8;j++)
			{
		...
		if (ctx->encrypt) TINYCLR_SSL_MEMCPY(ctx->iv,out_ptr,8);
		}
/* Process rest of buffer */
	if (i<inl)
		{
		gost_crypt_mesh(ctx->cipher_data,ctx->iv,ctx->buf);
		if (!ctx->encrypt) TINYCLR_SSL_MEMCPY(ctx->buf+8,in_ptr,inl-i);
		for (j=0;i<inl;j++,i++)
			{
			out_ptr[j]=ctx->buf[j]^in_ptr[j];
			}			
		ctx->num = j;
		if (ctx->encrypt) TINYCLR_SSL_MEMCPY(ctx->buf+8,out_ptr,j);
		}
	else
		{
		ctx->num = 0;
...
int  gost89_get_asn1_parameters(EVP_CIPHER_CTX *ctx,ASN1_TYPE *params)
	...
	TINYCLR_SSL_MEMCPY(ctx->oiv, gcp->iv->data, len);
...
int gost_imit_init_cpa(EVP_MD_CTX *ctx)
	{
	struct ossl_gost_imit_ctx *c = (ossl_gost_imit_ctx *)ctx->md_data;
	TINYCLR_SSL_MEMSET(c->buffer,0,16);
	c->count = 0;
	c->bytes_left=0;
	c->key_meshing=1;
	gost_init(&(c->cctx),&Gost28147_CryptoProParamSetA);
...
int gost_imit_update(EVP_MD_CTX *ctx, const void *data, size_t count)
	...
	if (bytes>0)
		{
		TINYCLR_SSL_MEMCPY(c->partial_block,p,bytes);
		}	
	c->bytes_left=bytes;
	return 1;
	}
...
int gost_imit_ctrl(EVP_MD_CTX *ctx,int type, int arg, void *ptr)
	...
	switch (type)
		...
		gost_key(&(((struct ossl_gost_imit_ctx*)(ctx->md_data))->cctx),(const byte*)ptr)	;
...
int gost_imit_copy(EVP_MD_CTX *to,const EVP_MD_CTX *from)
	{
	TINYCLR_SSL_MEMCPY(to->md_data,from->md_data,sizeof(struct ossl_gost_imit_ctx));
	return 1;
	}
...
int gost_imit_cleanup(EVP_MD_CTX *ctx)
	{
	TINYCLR_SSL_MEMSET(ctx->md_data,0,sizeof(struct ossl_gost_imit_ctx));
	return 1;
	}
