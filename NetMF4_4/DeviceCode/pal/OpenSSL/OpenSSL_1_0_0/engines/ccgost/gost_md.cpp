...
int gost_digest_init(EVP_MD_CTX *ctx) 
	{
	struct ossl_gost_digest_ctx *c = (ossl_gost_digest_ctx*)ctx->md_data;
	TINYCLR_SSL_MEMSET(&(c->dctx),0,sizeof(gost_hash_ctx));
...
int gost_digest_update(EVP_MD_CTX *ctx,const void *data,size_t count) 
	{
	return hash_block((gost_hash_ctx *)ctx->md_data,(const byte*)data,count);	
	}
...
int gost_digest_copy(EVP_MD_CTX *to,const EVP_MD_CTX *from) 
	{
	struct ossl_gost_digest_ctx *md_ctx=(ossl_gost_digest_ctx*)to->md_data;
	if (to->md_data && from->md_data) {
		TINYCLR_SSL_MEMCPY(to->md_data,from->md_data,sizeof(struct ossl_gost_digest_ctx));
...
int gost_digest_cleanup(EVP_MD_CTX *ctx) 
	{
	if (ctx->md_data)
	TINYCLR_SSL_MEMSET(ctx->md_data,0,sizeof(struct ossl_gost_digest_ctx));
	return 1;
	}	
