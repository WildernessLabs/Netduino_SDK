...
static int walk_nodename(di_node_t node, di_node_name_t di_node_name)
	...
	if (!TINYCLR_SSL_STRCMP (name,"SUNW,UltraSPARC") ||
	    !TINYCLR_SSL_STRNCMP(name,"SUNW,UltraSPARC-I",17))  /* covers II,III,IV */
		{
	...
	else if (!TINYCLR_SSL_STRNCMP(name,"SUNW,UltraSPARC",15))
		{
		OPENSSL_sparcv9cap_P &= ~SPARCV9_TICK_PRIVILEGED;
...
void OPENSSL_cpuid_setup(void)
	...
	if ((e=TINYCLR_SSL_GETENV("OPENSSL_sparcv9cap")))
		{
	...
	if (sysinfo(SI_MACHINE,si,sizeof(si))>0)
		{
		if (TINYCLR_SSL_STRCMP(si,"sun4v"))
...
void OPENSSL_cpuid_setup(void)
	...
	TINYCLR_SSL_MEMSET(&common_act,0,sizeof(common_act));
