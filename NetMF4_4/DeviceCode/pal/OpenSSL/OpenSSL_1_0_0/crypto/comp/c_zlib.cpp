...
static void* zlib_zalloc(void* opaque, unsigned int no, unsigned int size)
{
	...
	if (p)
		TINYCLR_SSL_MEMSET(p, 0, no*size);
	return p;
}
...
static int zlib_stateful_compress_block(COMP_CTX *ctx, unsigned char *out,
	...
	if (err != Z_OK)
		return -1;
#ifdef DEBUG_ZLIB
	TINYCLR_SSL_FPRINTF(OPENSSL_TYPE__FILE_STDERR,"compress(%4d)->%4d %s\n",
...
static int zlib_stateful_expand_block(COMP_CTX *ctx, unsigned char *out,
	...
	if (err != Z_OK)
		return -1;
#ifdef DEBUG_ZLIB
	TINYCLR_SSL_FPRINTF(OPENSSL_TYPE__FILE_STDERR,"expand(%4d)->%4d %s\n",
...
static int zlib_compress_block(COMP_CTX *ctx, unsigned char *out,
	...
	if (clear)
		{
		out[0]=0;
		TINYCLR_SSL_MEMCPY(&(out[1]),in,ilen);
		l=ilen+1;
		}
#ifdef DEBUG_ZLIB
	TINYCLR_SSL_FPRINTF(OPENSSL_TYPE__FILE_STDERR,"compress(%4d)->%4d %s\n",
...
static int zlib_expand_block(COMP_CTX *ctx, unsigned char *out,
	...
	if (in[0])
		...
		{
		TINYCLR_SSL_MEMCPY(out,&(in[1]),ilen-1);
		l=ilen-1;
		}
#ifdef DEBUG_ZLIB
        TINYCLR_SSL_FPRINTF(OPENSSL_TYPE__FILE_STDERR,"expand  (%4d)->%4d %s\n",
