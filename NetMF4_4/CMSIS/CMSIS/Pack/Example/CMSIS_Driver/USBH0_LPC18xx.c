...
#define ARM_USBH_EHCI_DRIVER_VERSION ARM_DRIVER_VERSION_MAJOR_MINOR(2,02)  // USBH driver version
...
static int32_t USBH_HCI_Initialize (ARM_USBH_HCI_Interrupt_t cb_interrupt) {
  ...
  if (USB0_state & USBH_DRIVER_INITIALIZED) { return ARM_DRIVER_OK;    }
...
static int32_t USBH_HCI_Uninitialize (void) {
  ...
  if (!(USB0_state & USBH_DRIVER_INITIALIZED)) { return ARM_DRIVER_OK;    }
  if (  USB0_state & USBH_DRIVER_POWERED     ) { return ARM_DRIVER_ERROR; }
...
static int32_t USBH_HCI_PowerControl (ARM_POWER_STATE state) {
  ...
  if (!(USB0_state & USBH_DRIVER_INITIALIZED)) { return ARM_DRIVER_ERROR; }

  switch (state) {
    case ARM_POWER_OFF:
      if (!(USB0_state & USBH_DRIVER_POWERED)) { return ARM_DRIVER_OK; }
      USB0_state &= ~USBH_DRIVER_POWERED;
