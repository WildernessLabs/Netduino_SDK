...
{
    public class Pkcs7 : PaddingTestBase, IMFTestInterface
    {
        protected override byte[][] GetData()
        {
			...
			new byte[] {0xFF, 0xFF, 0xFF, 0x05, 0x05, 0x05, 0x05, 0x05},
			...
			new byte[] {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
						0xFF, 0xFF, 0xFF, 0xFF, 0x04, 0x04, 0x04, 0x04},
		};
        }
        ...
        protected override bool CheckPadding(Session session, byte[] data, byte[] padding)
        {
            ...
            // each byte of the pad should be the length of the pad
            for (int i = 0; i < padding.Length; i++)
            {
                if (padding[i] != padLen)
                {
                    Log.Comment("ERROR! Padding Byte != " + padLen + " - " + padding[i]);
                    return false;
                }
            }
