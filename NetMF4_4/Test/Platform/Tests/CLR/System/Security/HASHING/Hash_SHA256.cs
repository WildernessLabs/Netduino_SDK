...
{
    class Hash_SHA256 : IMFTestInterface
    {
        bool m_isEmulator;
        ...
        static Boolean Test(Session session, Session session2)
        {
            ...
            using (HashAlgorithm rc21 = new HashAlgorithm(HashAlgorithmType.SHA256, session ))
            using (HashAlgorithm rc22 = new HashAlgorithm(HashAlgorithmType.SHA256, session2))
            {
        ...
        [TestMethod]
        public MFTestResults Hash_SHA256_Test()
        {
            bool bRes = true;
            ...
            try
            {
                if (m_isEmulator)
                {
                    ...
                    using (Session sess2 = new Session("Emulator_Crypto", MechanismType.SHA256))
                    {
