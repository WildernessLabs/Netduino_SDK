// Tests the ProduceLegacyHmacValues property with the legacyHMACMode appcompat switch set 
// (legacy result).  If the property is set it should always override the config setting.
//
using System;
...
class TestClass
{
	...
	public static int Main()
	{
		...
		// HMAC-SHA-384 with legacy property not set -> default result (legacy)
		//
		HMACSHA384 hm384Default = new HMACSHA384(key);
		byte[] result384Default = hm384Default.ComputeHash(data);
		
		if (!CompareBytes(result384Default, hash384Legacy))
		{
		...
		// HMAC-SHA-512 with legacy property not set -> default result (legacy)
		//
		HMACSHA512 hm512Default = new HMACSHA512(key);
		byte[] result512Default = hm512Default.ComputeHash(data);

		if (!CompareBytes(result512Default, hash512Legacy))
		{
