  ...
  <xsl:template match="/msbuild:Project/msbuild:Import">
    ...
    <xsl:choose>
      <!-- Samples like SPITemperatureDevice could use the standard Microsoft.CSharp.Targets instead of Emulator.targets, but I don't think it
           hurts anything.
      -->
      <xsl:when test="@Project = '$(SPOCLIENT)\tools\Targets\Microsoft.SPOT.CSharp.Targets' or
                      @Project = '$(SPOCLIENT)\tools\Targets\Microsoft.SPOT.Test.CSharp.Targets' or
                      @Project = '$(SPOCLIENT)\Framework\IDE\Targets\v{$ShortVersion}\CSharp.Targets'">
        <xsl:copy>
          <xsl:attribute name="Project">$(NetMfTargetsBaseDir)$(TargetFrameworkVersion)\CSharp.Targets</xsl:attribute>
        </xsl:copy>
      </xsl:when>
      <xsl:when test="@Project = '$(SPOCLIENT)\tools\Targets\Microsoft.SPOT.CSharp.Host.Targets' or
                      @Project = '$(SPOCLIENT)\tools\Targets\Microsoft.SPOT.Test.CSharp.Host.Targets'">
        <xsl:copy>
